...
UPDATE etl_webservice_request SET body_file = CONCAT(CONVERT(date,created_on),'/', LOWER(CONVERT(VARCHAR(MAX),oid,1)), '/request.json');
...
ALTER TABLE etl_webservice_request ADD http_body NVARCHAR(MAX);

/*
@plugin.WriteFilesToSqlRows(
    "SELECT body_file, LOWER(CONVERT(VARCHAR(MAX),oid,1)) as oid FROM etl_webservice_request WHERE body_file IS NOT NULL;",
    "UPDATE etl_webservice_request SET http_body = [#content#] WHERE LOWER(CONVERT(VARCHAR(MAX),oid,1))) = [#key#];",
    'body_file',
	'oid',
	'axenox.ETL.webservice_request_storage'
);

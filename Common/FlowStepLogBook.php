...
class FlowStepLogBook extends DataLogBook
{
    ...
    public function __construct(string $title, ETLStepInterface $step, ETLStepDataInterface $stepData)
    {
        ...
        $this->addLine(
            PhpClassDataType::findClassNameWithoutNamespace($step) . 
    ...
    /**
     * The flow step logbook will show a profiler tab in addition to the regular data logbook tabs
     * 
     * @see DataLogBook::createDebugWidget()
     */
    public function createDebugWidget(DebugMessage $debugWidget)
    {
        $debugWidget = parent::createDebugWidget($debugWidget);
        $debugWidget = $this->getStepData()->getProfiler()->createDebugWidget($debugWidget);
        return $debugWidget;
    }
}